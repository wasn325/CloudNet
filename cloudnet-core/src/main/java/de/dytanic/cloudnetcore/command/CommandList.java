...
public final class CommandList extends Command {
    ...
    public void onExecuteCommand(final CommandSender sender, final String[] args) {
        ...
        for (final Wrapper wrapper : CloudNet.getInstance().getWrappers().values()) {
            ...
            sender.sendMessage("Info: CPU Usage: " + NetworkUtils.DECIMAL_FORMAT.format(wrapper.getCpuUsage()) + "/100% | Memory: " +
            ...
            for (final MinecraftServer proxyServer : wrapper.getServers().values()) {
                sender.sendMessage("Server [" + proxyServer.getServerId() + "] @" + proxyServer.getServerInfo().getHost() + " | " +
                                   proxyServer.getServerInfo().getOnlineCount() + NetworkUtils.SLASH_STRING +
                                   proxyServer.getServerInfo().getMaxPlayers() + " | State: " +
                                   (proxyServer.getChannel() != null ? "connected" : "not connected"));
            }
            sender.sendMessage(" ");
        }
        ...
        for (final CloudPlayer cloudPlayer : CloudNet.getInstance().getNetworkManager().getOnlinePlayers().values()) {
            sender.sendMessage("* " + cloudPlayer.getUniqueId() + '#' + cloudPlayer.getName() + " - " + cloudPlayer.getProxy() + ':' +
