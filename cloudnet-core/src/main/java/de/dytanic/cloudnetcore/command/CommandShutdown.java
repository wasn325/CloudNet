    ...
    public void onExecuteCommand(final CommandSender sender, final String[] args) {
        if (args.length == 2) {
            ...
            if (args[0].equalsIgnoreCase("server") || args[0].equalsIgnoreCase("-s")) {
                final MinecraftServer proxyServer = CloudNet.getInstance().getServer(args[1]);
                if (proxyServer != null) {
                    proxyServer.getWrapper().stopServer(proxyServer);
                    sender.sendMessage("Server " + args[1] + " was stopped!");
                } else {
                    final CloudServer proxyServers = CloudNet.getInstance().getCloudGameServer(args[1]);
            ...
            if (args[0].equalsIgnoreCase("proxy") || args[0].equalsIgnoreCase("-p")) {
                final ProxyServer proxyServer = CloudNet.getInstance().getProxy(args[1]);
                if (proxyServer != null) {
