...
public class BungeeCordChannel implements Listener {
    ...
    @EventHandler
    public void handleChannelIncomingMessage(final ProxiedSubChannelMessageEvent e) {
        if (e.getChannel().equalsIgnoreCase("ban-system")) {
            if (e.getMessage().equalsIgnoreCase("kick")) {
                final ProxiedPlayer proxiedPlayer = ProxyServer.getInstance().getPlayer(e.getDocument().getObject("uuid", UUID.class));
